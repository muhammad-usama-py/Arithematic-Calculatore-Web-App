...
export class Rational {
  ...
  static fromInteger(n: number | bigint) {
  ...
  static fromDecimalString(s: string): Rational {
    ...
    if (!m) throw new Error('Invalid numeric format');
    ...
    if (intPart.length + fracPart.length > MAX_TOTAL_DIGITS) throw new Error('Numeric literal too long');
    if (Math.abs(expPart) > 1000) throw new Error('Exponent magnitude too large');
  ...
  toDecimalString(maxFractionDigits = 20): string {
  ...
  toRoundedDecimalString(digits: number): string {
