import { Value, ExpressionNode } from './types';
import { exprToReadable, isTerminatingDenominator } from './utils';

export function generateExplanationForValue(value: Value): { steps: string[]; explanation: string } {
  ...
  if (p.source === 'user') {
    ...
    if (p.roundedDigits != null) steps.push(`Step 4: The input was explicitly rounded to ${p.roundedDigits} digits for display.`);
    return { steps, explanation: steps.join('\n') };
  }

  if (p.source === 'calc') {
    steps.push(`Step 1: The calculator evaluated the expression ${exprToReadable(p.expr)}.`);
    const denom = value.exact.denominator;
    // If expression contains literal operands, mention whether they were user-entered
    function gatherLiterals(node?: ExpressionNode, acc: Array<{ raw: string; source?: string }> = []) {
      if (!node) return acc;
      if (node.type === 'literal') {
        acc.push({ raw: node.raw, source: (node as any).source });
        return acc;
      }
    ...
    } else if (isTerminatingDenominator(denom)) {
...
export function generateExplanationForOperation(result: Value, a: Value, b: Value, op: string): { steps: string[]; explanation: string } {
  const steps: string[] = [];
  // Describe inputs
  ...
  if (a.provenance.source === 'calc' && a.provenance.expr) {
    steps.push(`Step 2: The first operand originated from a previous calculation: ${exprToReadable(a.provenance.expr)}.`);
  ...
  if (b.provenance.source === 'calc' && b.provenance.expr) {
    steps.push(`Step ${baseStep}: The second operand originated from a previous calculation: ${exprToReadable(b.provenance.expr)}.`);
  ...
  if (result.exact.denominator === 1n) {
    ...
    if (!isTerminatingDenominator(result.exact.denominator)) {
